...
`), 0644)
	...
	execCmd := exec.Command("docker", "compose", "exec", "-T", "llm-agent", "bash", "-c", "git --version && command -v forgejo-mcp && forgejo-mcp --help >/dev/null && test -f /root/.codex/config.toml && test -f /root/.gemini/settings.json && grep -q '/usr/local/bin/forgejo-mcp' /root/.codex/config.toml && grep -q '/usr/local/bin/forgejo-mcp' /root/.gemini/settings.json && grep -q 'FORGEJO_URL' /root/.codex/config.toml && grep -q 'FORGEJO_URL' /root/.gemini/settings.json")
